...
public class GameTestScript
{
    ...
    public IEnumerator GameTestScriptWithEnumeratorPasses()
    {
        ...
        // 게임 제어 관련 버튼 확인
        _leftMoveButton = GameObject.Find("Left Move Button");
        Assert.IsNotNull(_leftMoveButton, "Left Move Button is null");
        _rightMoveButton = GameObject.Find("Right Move Button");
    ...
    private void MoveCar(Vector3 tragetPosition)
    {
        if (Mathf.Abs(tragetPosition.x - _carController.transform.position.x) < 0.1f)
        {
            MoveButtonUp(_rightMoveButton);
            MoveButtonUp(_leftMoveButton);
            return;
        ...
        if (tragetPosition.x < _carController.transform.position.x)
        {
            // 왼쪽으로 이동
            MoveButtonDown(_leftMoveButton);
            MoveButtonUp(_rightMoveButton);
        }
        else if (tragetPosition.x > _carController.transform.position.x)
        {
            // 오른쪽으로 이동
